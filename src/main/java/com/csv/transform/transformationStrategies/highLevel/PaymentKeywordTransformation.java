...
import com.csv.transform.transformationStrategies.lowLevel.map.MapByPaymentKeywordColumn;
...
@Component
public class PaymentKeywordTransformation implements TransformationStrategy<CsvTable, Pair<CsvTable, ExcelIndexTable>> {
    @Autowired
    NormalizeColumns normalizeColumns;
    @Autowired
    RemoveEmptyColumns removeEmptyColumns;
    @Autowired
    MapByPaymentKeywordColumn mapByPaymentKeywordColumn;
    @Autowired
    ...
    public Pair<CsvTable, ExcelIndexTable> transform(CsvTable input) {
        ...
        var tables = mapByPaymentKeywordColumn.transform(removed);
