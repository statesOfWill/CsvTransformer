...
@Component
public class MapByDateColumn implements TransformationStrategy<CsvTable, CsvTables> {
    @Autowired
    ObjectMapperWrapper mapper;
    @Override
    public CsvTables transform(CsvTable source) {
        var target = new ArrayList<CsvTable>();
        var dateRanges = source.getDateRanges();
        ...
        for(Range<LocalDate> range : dateRanges) {
            ...
            for (int i = 0; i < source.rowSize(); i++) {
                ...
                var cell = mapper.objToDateCell(row.get(mappingColIndex));
