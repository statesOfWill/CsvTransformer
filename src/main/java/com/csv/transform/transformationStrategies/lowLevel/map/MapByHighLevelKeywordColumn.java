...
@Component
public class MapByHighLevelKeywordColumn implements TransformationStrategy<CsvTable, CsvTables> {
    @Autowired
    ObjectMapperWrapper mapper;
    @Override
    public CsvTables transform(CsvTable source) {
        var target = new ArrayList<CsvTable>();
        var keywords = source.getHighLevelKeywords();
        ...
        for(String keyword : keywords) {
            ...
            for (int i = 0; i < source.rowSize(); i++) {
                ...
                var cell = mapper.objToStringCell(row.get(mappingColIndex));
